...
-- Create issue history table if it doesn't exist
...
DO $$ 
BEGIN
    DROP POLICY IF EXISTS "Allow authenticated users to read issue history" ON issue_history;
    DROP POLICY IF EXISTS "Allow authenticated users to insert issue history" ON issue_history;
END $$;
...
-- Create or replace function to automatically add history entry when issue status changes
